...
def main():
    ...
    cfg = ExtractConfig(
        ...
        extract_mode="last_gen",
        do_sample=False,
        temperature=0.0,
        top_p=1.0,
        debug_hook=bool(args.debug_hook),
    )
    # vector_extractor.py uses cfg.max_new_tokens in generate()
    setattr(cfg, "max_new_tokens", int(args.max_new_tokens))
        ...
        records.append(
            {
                ...
                "layer_select": args.layer_select,
            }
        )
    ...
    for _, row in movies_unique.iterrows():
        ...
        v0_vec, meta = extractor._extract_hidden_vector(prompt)
    ...
    plot_hist(within_arr, "Movie–movie cosine distance (within genre) [v0]", os.path.join(args.out_dir, "hist_movie_within_genre.png"))
    plot_hist(cross_arr, "Movie–movie cosine distance (cross genre) [v0]", os.path.join(args.out_dir, "hist_movie_cross_genre.png"))
    ...
    plot_hist(within_out_arr, "Output match distance (within genre) [0=same,1=diff]", os.path.join(args.out_dir, "hist_output_match_within.png"))
    plot_hist(cross_out_arr, "Output match distance (cross genre) [0=same,1=diff]", os.path.join(args.out_dir, "hist_output_match_cross.png"))

    print(f"Saved all outputs to: {args.out_dir}")
    ...
    # match_distance is binary, so mean is just "fraction different"
    print("Output similarity summary (match-distance; 0=same title, 1=different title):")
